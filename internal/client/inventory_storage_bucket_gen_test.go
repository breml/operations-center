...
// Test_ResyncStorageBucket only covers the error paths, which are
// handled before the Incus API of the cluster is contacted.
func Test_ResyncStorageBucket(t *testing.T) {
	d := daemonSetup(t)
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := tc.client.ResyncStorageBucket(t.Context(), tc.tcNameArg)
