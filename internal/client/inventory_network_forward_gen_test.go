...
// Test_ResyncNetworkForward only covers the error paths, which are
// handled before the Incus API of the cluster is contacted.
func Test_ResyncNetworkForward(t *testing.T) {
	d := daemonSetup(t)
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := tc.client.ResyncNetworkForward(t.Context(), tc.tcNameArg)
