...
// mediaGUIDs derives the GPT identifiers of an image from its ID, so that the
// very same certificates produce the very same image.
func mediaGUIDs(id string) (uuid.UUID, uuid.UUID) {
	return derivedUUID(id, "disk"), derivedUUID(id, "partition")
}

func derivedUUID(id string, purpose string) uuid.UUID {
	sum := sha256.Sum256([]byte(id + "\x00" + purpose))
