...
func (m *Media) generate(ctx context.Context, id string, loader bootLoader, certificates provisioning.SecureBootCertificates) (err error) {
	...
	esp, err := os.Open(espFile)
	if err != nil {
		return fmt.Errorf("Failed to open the EFI system partition: %w", err)
	}

	defer func() { _ = esp.Close() }()

	espInfo, err := esp.Stat()
	if err != nil {
		return fmt.Errorf("Failed to stat the EFI system partition: %w", err)
	}

	return m.write(id, func(w io.Writer) error {
		diskGUID, partitionGUID := mediaGUIDs(id)

		return writeISO(w, esp, espInfo.Size(), diskGUID, partitionGUID)
	})
}
